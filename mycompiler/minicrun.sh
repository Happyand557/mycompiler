#!/bin/bash
workpath=$(pwd)
export LD_LIBRARY_PATH="${workpath}/IRCompiler/Linux-x86_64/Ubuntu"
APPFIX=""
WINOS=0
...
function dotestcase() 
{
    ...
    symbol="${TESTNAME}_symbol.txt"
    ast="${TESTNAME}_ast.png"
    function1="${TESTNAME}_function.png"
    #CMDNAME="./minic${APPFIX} -S -I -o ${IRFILE} ${CFILE}"
    CMDNAME="./build/minic${APPFIX} -i -o ${IRFILE} ${CFILE}"
    ...
    if [[ $OSKind == "Linux" ]]; then
        LinuxDisID=`lsb_release -i`
        CMDNAME="./IRCompiler/${OSKind}-${OS3264}/Ubuntu/IRCompiler${APPFIX} -R ${IRFILE}"
    else
    ...
    if [ $? -ne 0 ]; then
        ...
        rm -f ${RESULTFILE}
        #rm -f ${IRFILE}
        ./build/minic -a -o ${ast} ${CFILE}
        ./build/minic -c main -o ${function1} ${CFILE}
        ./build/minic -s -o ${symbol} ${CFILE}
    fi
}
